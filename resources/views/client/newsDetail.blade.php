...
@section('content')
    <div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <img alt="{{ $news->title }}" src="{{ asset('storage/' . $news->thumbnail) }}" class="h-56 w-full object-cover" />

        <div class="bg-white p-4 sm:p-6">
            <h3 class="mt-0.5 text-3xl text-gray-900">{{ $news->title }}</h3>
            <div class="flex gap-2 items-center mt-2">
                <time datetime="2022-10-10" class="block text-xs text-gray-500">
                    {{ \Carbon\Carbon::parse($news->published_at)->format('l, d-m-Y') }}
                </time>
                <span class="text-gray-500 text-xs">| Oleh {{ $news->user->name }}</span>
            </div>
